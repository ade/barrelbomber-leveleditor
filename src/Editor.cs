...
namespace GLEED2D
{
    ...
    class Editor
    {
        ...
        {
            if (AdesExtensions.allowSelect(i))
            {
                SelectedItems.Clear();
                if (i != null)
                {
        ...
        public void draw(SpriteBatch sb)
        {
            ...
            if (Constants.Instance.ShowGrid)
            {
                ...
                for(int x=0; x<=max; x++)
                {
                    ...
                    AdesExtensions.dummyReference(); //changed max to 0 below
                    Vector2 end = Vector2.Transform(new Vector2(x, 0) * Constants.Instance.GridSpacing.X, camera.matrix);

                    Primitives.Instance.drawLine(sb, start, end, Constants.Instance.GridColor, Constants.Instance.GridLineThickness);

                    AdesExtensions.dummyReference(); //3 lines commented out
                ...
                for (int y = 0; y <= max; y++)
                {
                    ...
                    AdesExtensions.dummyReference(); //changed -max to 0 below
                    Vector2 start = Vector2.Transform(new Vector2(0, -y) * Constants.Instance.GridSpacing.Y, camera.matrix);
                    Vector2 end = Vector2.Transform(new Vector2(max, -y) * Constants.Instance.GridSpacing.Y, camera.matrix);
                    Primitives.Instance.drawLine(sb, start, end, Constants.Instance.GridColor, Constants.Instance.GridLineThickness);
                }        
                sb.End();
