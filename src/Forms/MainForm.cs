...
namespace GLEED2D
{
    public partial class MainForm : Form
    {
        ...
        private void FileOpen(object sender, EventArgs e)
        {
            ...
            opendialog.InitialDirectory = AdesExtensions.getSavePath();
            opendialog.Filter = "XML Files (*.xml)|*.xml";
            if (opendialog.ShowDialog() == DialogResult.OK)
            {
                loadLevel(opendialog.FileName);
                
                AdesExtensions.setSavePath(opendialog.FileName);
            }
        }
        ...
        {
            SaveFileDialog dialog = new SaveFileDialog();
            dialog.InitialDirectory = AdesExtensions.getSavePath();
            dialog.Filter = "XML Files (*.xml)|*.xml";
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                saveLevel(dialog.FileName);
                AdesExtensions.setSavePath(dialog.FileName);
            }
        }
        ...
        private void RunLevel(object sender, EventArgs e)
        {
            if (Constants.Instance.RunLevelStartApplication)
            {
                ...
                {
                    String tempFileName = Application.ExecutablePath.Substring(0, Application.ExecutablePath.LastIndexOf("\\")) + "\\$playtest_temp.xml";
                    AdesExtensions.exportLevel(tempFileName, Editor.Instance.level);
                    String appFile = Constants.Instance.RunLevelApplicationToStart;
                    ProcessStartInfo psi = new ProcessStartInfo(appFile);
                    psi.WorkingDirectory = appFile.Substring(0, appFile.LastIndexOf("\\"));
        ...
        private void exportLayerToPrefabToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        {
            AdesExtensions.onSelectGridProperty();
        }
        ...
        {
            AdesExtensions.quickExport();
        }

        private void exportAsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            AdesExtensions.exportLevelDialog();
        }

        private void exportLayersAsPrefabsToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        {
            ListViewItem lvi = (ListViewItem)e.Item;
            if (lvi.Text == "weldjoint" || lvi.Text == "revolutejoint")
            {

                string file = Application.StartupPath + "\\" + (string)lvi.Tag;
                lvi.Name = file;
                if (!File.Exists(file))
                {
                    MessageBox.Show("file missing: " + file);
                    return;
                }
                //toolStripStatusLabel1.Text = lvi.ToolTipText;
                //Bitmap bmp = new Bitmap(listView2.LargeImageList.Images[lvi.ImageKey]);
                //dragcursor = new Cursor(bmp.GetHicon());
